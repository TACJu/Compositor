    ...
    def memory_efficient_forward(self, outputs, targets, prefix="part_"):
        ...
        for b in range(bs):
            with autocast(enabled=False):
                class_similarity = compute_class_similarity(outputs[prefix + "pred_logits"][b].float(), targets[b][prefix + "labels"])
            out_mask = outputs[prefix + "pred_masks"][b].flatten(1)  # [num_queries, H_pred, W_pred]
            # gt masks are already padded when preparing target
            tgt_mask = targets[b][prefix + "masks"].to(out_mask).flatten(1)
            with autocast(enabled=False):

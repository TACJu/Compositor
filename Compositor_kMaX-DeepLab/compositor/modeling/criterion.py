...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_part_classes, num_object_classes, matcher, weight_dict, eos_coef, losses, share_final_matching,
    ...
    def loss_labels(self, outputs, targets, prefix="part_"):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert prefix + "pred_logits" in outputs
        src_logits = outputs[prefix + "pred_logits"] # B x N x C
        target_classes = targets[prefix + "labels"] # B x N
        pq_loss_class_weight = targets[prefix + "pq_loss_class_weight"]
        losses = {prefix + "loss_ce": focal_cross_entropy_loss(src_logits, target_classes, pq_loss_class_weight)}
        return losses
    
    def loss_masks(self, outputs, targets, prefix="part_"):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        targets dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        src_masks = outputs[prefix + "pred_masks"] # B x N x H x W
        target_masks = targets[prefix + "masks"]
        pq_loss_mask_weight = targets[prefix + "pq_loss_mask_weight"]
        pixel_gt_void_mask = targets[prefix + "pixel_gt_void_mask"]
        ...
        losses = {
            prefix + "loss_mask": softmax_ce_loss(src_masks, target_masks, pixel_gt_void_mask, masking_void_pixel=self.masking_void_pixel),
            prefix + "loss_dice": dice_loss(src_masks, target_masks, pixel_gt_void_mask, pq_loss_mask_weight, masking_void_pixel=self.masking_void_pixel),
        }

        return losses

    def loss_pixels(self, outputs, targets, prefix="part_"):
        pixel_feature = outputs["pixel_feature"]
        target_masks = targets[prefix + "masks"]
        pixel_gt_void_mask = targets[prefix + "pixel_gt_void_mask"]
        inverse_gt_mask_area = targets[prefix + "inverse_gt_mask_area"]

        losses = {prefix + "loss_pixel_insdis": pixelwise_insdis_loss(
    ...
    def get_loss(self, loss, outputs, targets, prefix="part_"):
        loss_map = {
            'part_labels': self.loss_labels,
            'part_masks': self.loss_masks,
            'part_pixels': self.loss_pixels,
            'object_labels': self.loss_labels,
            'object_masks': self.loss_masks,
            'object_pixels': self.loss_pixels,
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        return loss_map[loss](outputs, targets, prefix=prefix)

    @torch.no_grad()
    def process_gt(self, outputs, targets, indices, matched_dice, matched_cls_prob, prefix="part_"):
        # Permute&Pad Pred&GT for loss compuation.
        # By controling process_gt, we can share the matching results for all preds.
        src_idx = self._get_src_permutation_idx(indices)

        src_masks = outputs[prefix + "pred_masks"].detach() # B x N x H x W

        # Pad and permute the target_mask to B x N x H x W
        target_masks = torch.zeros_like(src_masks)
        target_masks_o = torch.cat([t[prefix + "masks"][J] for t, (_, J) in zip(targets, indices)]).to(target_masks)
        ...
        target_classes_o = torch.cat([t[prefix + "labels"][J] for t, (_, J) in zip(targets, indices)])
        # This serves as a padding.
        if prefix == 'part_':
            target_classes = torch.full(
                src_logits.shape[:2], self.num_part_classes, dtype=torch.int64, device=src_logits.device
            )
        elif prefix == 'object_':
            target_classes = torch.full(
                src_logits.shape[:2], self.num_object_classes, dtype=torch.int64, device=src_logits.device
            )
    ...
    def forward(self, outputs, targets):
        ...
        part_indices, part_matched_dice, part_matched_cls_prob = self.matcher(outputs_without_aux, targets, prefix='part_')
        object_indices, object_matched_dice, object_matched_cls_prob = self.matcher(outputs_without_aux, targets, prefix='object_')
        # Pad GT to the same number of prediction.
        part_processed_targets = self.process_gt(outputs, targets, part_indices, part_matched_dice, part_matched_cls_prob, prefix='part_')
        object_processed_targets = self.process_gt(outputs, targets, object_indices, object_matched_dice, object_matched_cls_prob, prefix='object_')
        # Merge part and object targets.
        processed_targets = {}
        for k in part_processed_targets:
            processed_targets['part_' + k] = part_processed_targets[k]
        for k in object_processed_targets:
            processed_targets['object_' + k] = object_processed_targets[k] 
        # Compute all the requested losses
        losses = {}
        for loss in self.losses:
            if loss.startswith('part_'):
                losses.update(self.get_loss(loss, outputs, processed_targets, prefix='part_'))
            elif loss.startswith('object_'):
                losses.update(self.get_loss(loss, outputs, processed_targets, prefix='object_'))

        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                # We share matching results across predictions.
                if not self.share_final_matching:
                    part_indices, part_matched_dice, part_matched_cls_prob = self.matcher(aux_outputs, targets, prefix='part_')
                    object_indices, object_matched_dice, object_matched_cls_prob = self.matcher(aux_outputs, targets, prefix='object_')
                    part_processed_targets = self.process_gt(aux_outputs, targets, part_indices, part_matched_dice, part_matched_cls_prob, process_semantic=False, prefix='part_')
                    object_processed_targets = self.process_gt(aux_outputs, targets, object_indices, object_matched_dice, object_matched_cls_prob, process_semantic=False, prefix='object_')
                    # Merge part and object targets.
                    processed_targets = {}
                    for k in part_processed_targets:
                        processed_targets['part_' + k] = part_processed_targets[k]
                ...
                for loss in self.losses:
                    if loss.startswith('part_'):
                        l_dict = self.get_loss(loss, aux_outputs, processed_targets, prefix='part_')
                    else:
                        l_dict = self.get_loss(loss, aux_outputs, processed_targets, prefix='object_')
    ...
    def __repr__(self):
        ...
        body = [
            ...
            "num_part_classes: {}".format(self.num_part_classes),
            "num_object_classes: {}".format(self.num_object_classes),
